...
void mixed_comparison_Dst_Dp() {
    ...
    TFile *output_file = new TFile("mixed_comparison_Dstar_Dp.root", "RECREATE");
    if (!output_file || output_file->IsZombie()) {
        std::cerr << "Error: Unable to create output file 'mixed_comparison_Dstar_Dp.root'.\n";
        delete output_file;
        return;
    }
    ...
    TLegend *legend = new TLegend(0.45, 0.40, 0.9, 0.65);
    ...
    if (!graphs.empty()) {
        ...
        graphs[0]->GetYaxis()->SetRangeUser(0.1, 2.55);
        graphs[0]->SetTitle("Ratio of D^{*+} / D^{+} for the different sources and rapidity ranges");
    ...
    c1->Update();
    c1->Write();
    c1->SaveAs("mixed_comparison_Dst_Dp.pdf");
    output_file->Close();
    delete output_file;
