...
void alice_ratioplot_indirect_from_Dst_D0_Dp() {
    ...
    TCut cuts1 = Form("%s_TRUEY >= 0.0 && %s_TRUEY < 1.0 && abs(Km_MC_MOTHER_ID) == 421 && abs(Hp_MC_MOTHER_ID) == 421 && (abs(%s_MC_MOTHER_ID) == 413 || abs(%s_MC_MOTHER_ID) == 423)", particle_type1, particle_type1, particle_type1, particle_type1);
    TCut cuts2 = Form("%s_TRUEY >= 0.0 && %s_TRUEY < 1.0 && abs(Km_MC_MOTHER_ID) == 411 && abs(H1_MC_MOTHER_ID) == 411 && abs(H2_MC_MOTHER_ID) == 411 && (abs(%s_MC_MOTHER_ID) == 413 || abs(%s_MC_MOTHER_ID) == 423)", particle_type2, particle_type2, particle_type2, particle_type2);
    ...
    TFile *output_file = new TFile(Form("alice_indirect_from_Dst_%s_%s.root", particle_type1, particle_type2), "RECREATE");
    if (!output_file || output_file->IsZombie()) {
        std::cerr << Form("Error: Unable to create output file 'alice_indirect_from_Dst_%s_%s.root'.\n", particle_type1, particle_type2);
        delete output_file;
        delete file1;
        delete file2;
        delete ratio_plot;
    ...
    auto legend = new TLegend(0.14,0.75,0.30,0.85);
    TString rapidity_range = "0.0 < y < 1.0";
    graph->GetXaxis()->SetTitle("p_{T} (MeV/c)");
    graph->GetXaxis()->CenterTitle();
    graph->GetYaxis()->CenterTitle();
    graph->GetYaxis()->SetTitle("D^{0} / D^{+} (from D^{*0} or D^{*+})");
    ...
    c2->Update();
    c2->Write();
    c2->SaveAs("indirect_from_Dst_ratios_D0_Dp_0y1.pdf");
    output_file->Close();
