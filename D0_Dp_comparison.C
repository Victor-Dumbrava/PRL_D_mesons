...
void D0_Dp_comparison() {
    ...
    // boolean params are: false/true (for D0/Dst), false/true (for ccbar/other sources), false/true (for lchb/alice) respectively
    openInputFiles(graphs, false, false, false); // D0, ccbar, lchb
    ...
    TFile *output_file = new TFile("comparison_D0_Dp.root", "RECREATE");
    if (!output_file || output_file->IsZombie()) {
        std::cerr << "Error: Unable to create output file 'comparison_D0_Dp.root'.\n";
        delete output_file;
        return;
    }
    ...
    TLegend *legend = new TLegend(0.7, 0.37, 0.89, 0.55);
    ...
    TLatex *t_ccbar = new TLatex(2.0,1.45,"#bf{from the feeddown of D^{*0} or D^{*+}}");
    TText *t_other = new TLatex(2.0,.5,"#bf{from c or #bar{c}}");
    ...
    c1->Write();
    c1->Draw();
    c1->SaveAs("comparison_D0_Dp.pdf");
    output_file->Close();
    delete output_file;
