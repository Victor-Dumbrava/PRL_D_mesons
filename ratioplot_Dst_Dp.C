...
void ratioplot_Dst_Dp() {
    ...
    for (int i = 0; i < num_rapidity_bins; ++i) {
        ...
        TCut cuts1 = Form("%s_TRUEY >= %f && %s_TRUEY < %f && abs(Km_MC_MOTHER_ID) == 421 && abs(H_soft_MC_MOTHER_ID) == 413 && abs(Hp_MC_MOTHER_ID) == 421", particle_type1, rapidity_bins[i], particle_type1, rapidity_bins[i+1]);
    ...
    for (int i = 0; i < 6; ++i) {
        c2->cd(i + 1);
        TGraph *graph = ratio_plots[i]->GetCalculationOutputGraph();
        auto legend = new TLegend(0.15,0.8,0.35,0.95);
    ...
    c2->Update();
    c2->Write();
    c2->SaveAs("ratios_Dst_Dp_2y5.pdf");
    output_file->Close();
    std::cout << "Done." << std::endl;
    delete output_file;
    delete file1;
