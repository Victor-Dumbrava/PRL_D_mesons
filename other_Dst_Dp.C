...
void other_Dst_Dp() {
    ...
    TFile *output_file = new TFile(Form("plotratio_other_%s_%s.root", particle_type1, particle_type2), "RECREATE");
    if (!output_file || output_file->IsZombie()) {
        std::cerr << Form("Error: Unable to create output file 'plotratio_other_%s_%s.root'.\n", particle_type1, particle_type2);
        delete output_file;
        delete file1;
        delete file2;
    ...
    c2->Update();
    c2->Write();
    c2->SaveAs("ratios_other_Dst_Dp_2y5.pdf");
    output_file->Close();
    std::cout << "Done." << std::endl;
    delete output_file;
    delete file1;
