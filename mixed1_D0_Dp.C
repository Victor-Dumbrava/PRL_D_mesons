...
void mixed1_D0_Dp() {
    ...
    for (int i = 0; i < num_rapidity_bins; ++i) {
        ...
        TCut cuts1 = Form("%s_TRUEY >= %f && %s_TRUEY < %f && abs(Km_MC_MOTHER_ID) == 421 && abs(Hp_MC_MOTHER_ID) == 421 && abs(%s_MC_MOTHER_ID) == 4", particle_type1, rapidity_bins[i], particle_type1, rapidity_bins[i+1], particle_type1);
    ...
    TFile *output_file = new TFile(Form("mixed1_%s_%s.root", particle_type1, particle_type2), "RECREATE");
    if (!output_file || output_file->IsZombie()) {
        std::cerr << Form("Error: Unable to create output file 'mixed1_%s_%s.root'.\n", particle_type1, particle_type2);
        delete output_file;
        delete file1;
        delete file2;
    ...
    c2->Update();
    c2->Write();
    c2->SaveAs("mixed1_D0_Dp_2y5.pdf");
    output_file->Close();
    std::cout << "Done." << std::endl;
    delete output_file;
    delete file1;
