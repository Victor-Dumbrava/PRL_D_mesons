...
void ratioplot_indirect_from_Dst_D0_Dp() {
    ...
    for (int i = 0; i < num_rapidity_bins; ++i) {
        ...
        TCut cuts1 = Form("%s_TRUEY >= %f && %s_TRUEY < %f && abs(Km_MC_MOTHER_ID) == 421 && abs(Hp_MC_MOTHER_ID) == 421 && (abs(%s_MC_MOTHER_ID) == 413 || abs(%s_MC_MOTHER_ID) == 423)", particle_type1, rapidity_bins[i], particle_type1, rapidity_bins[i+1], particle_type1, particle_type1);
        TCut cuts2 = Form("%s_TRUEY >= %f && %s_TRUEY < %f && abs(Km_MC_MOTHER_ID) == 411 && abs(H1_MC_MOTHER_ID) == 411 && abs(H2_MC_MOTHER_ID) == 411 && (abs(%s_MC_MOTHER_ID) == 413 || abs(%s_MC_MOTHER_ID) == 423)", particle_type2, rapidity_bins[i], particle_type2, rapidity_bins[i+1], particle_type2, particle_type2);
    ...
    TFile *output_file = new TFile(Form("indirect_from_Dst_%s_%s.root", particle_type1, particle_type2), "RECREATE");
    if (!output_file || output_file->IsZombie()) {
        std::cerr << Form("Error: Unable to create output file 'indirect_from_Dst_%s_%s.root'.\n", particle_type1, particle_type2);
        delete output_file;
        delete file1;
        delete file2;
    ...
    c2->Update();
    c2->Write();
    c2->SaveAs("indirect_from_Dst_ratios_D0_Dp_2y5.pdf");
    output_file->Close();
    std::cout << "Done." << std::endl;
    delete output_file;
    delete file1;
