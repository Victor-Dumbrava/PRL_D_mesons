...
void alice_direct_hadronization_Dst_Dp() {
    ...
    TFile *output_file = new TFile(Form("alice_direct_hadronization_plotratio_%s_%s.root", particle_type1, particle_type2), "RECREATE");
    if (!output_file || output_file->IsZombie()) {
        std::cerr << Form("Error: Unable to create output file 'alice_direct_hadronization_plotratio_%s_%s.root'.\n", particle_type1, particle_type2);
        delete output_file;
        delete file1;
        delete file2;
        delete ratio_plot;
    ...
    c2->Update();
    c2->Write();
    c2->SaveAs("ratio_direct_hadronization_Dst_Dp_0y1.pdf");
    output_file->Close();
