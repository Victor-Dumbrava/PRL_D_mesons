...
void mixed_comparison_D0_Dp() {
    ...
    TFile *output_file = new TFile("mixed_comparison_alice_D0_Dp.root", "RECREATE");
    if (!output_file || output_file->IsZombie()) {
        std::cerr << "Error: Unable to create output file 'mixed_comparison_alice_D0_Dp.root'.\n";
        delete output_file;
        return;
    }
    ...
    TLegend *legend = new TLegend(0.1, 0.45, 0.55, 0.70);
    ...
    c1->Update();
    c1->Write();
    c1->SaveAs("mixed_comparison_D0_Dp.pdf");
    output_file->Close();
    delete output_file;
