void openInputFiles(std::vector<TGraphAsymmErrors*>& graphs, int mixed = 0, bool Dst = false, bool indirect = false, bool alice = false) {
    ...
    if (mixed == 0) {
        ...
        if (alice) {
            fileName = Form("alice_mixed%i_D0_Dp.root", mixed);
        } else {
...
void mixed1_D0_Dp_comparison() {

    std::vector<TGraphAsymmErrors*> graphs;
    std::vector<TString> labels = { "2.0 < y < 2.5 (D^{0}: D^{*+}, D^{+}: c / #bar{c})",
                                    "2.5 < y < 3.0 (D^{0}: D^{*+}, D^{+}: c / #bar{c})",
                                    "2.0 < y < 2.5 (D^{0}: D^{*+} or D^{*0}, D^{+}: c / #bar{c})",
                                    "2.5 < y < 3.0 (D^{0}: D^{*+} or D^{*0}, D^{+}: c / #bar{c})",
                                    "2.0 < y < 2.5 (D^{0}: c / #bar{c}, D^{+}: D^{*+} or D^{*0})",
                                    "2.5 < y < 3.0 (D^{0}: c / #bar{c}, D^{+}: D^{*+} or D^{*0})" };
    openInputFiles(graphs, 3, false, false, false);
    openInputFiles(graphs, 2, false, false, false);
    ...
    TFile *output_file = new TFile("comparison_mixed1_D0_Dp.root", "RECREATE");
    if (!output_file || output_file->IsZombie()) {
        std::cerr << "Error: Unable to create output file 'comparison_mixed1_D0_Dp.root'.\n";
        delete output_file;
        return;
    }
    ...
    TLegend *legend = new TLegend(0.45, 0.35, 0.9, 0.65);
    ...
    c1->Update();
    c1->Write();
    c1->SaveAs("comparison_mixed1_D0_Dp.pdf");
    output_file->Close();
    delete output_file;
