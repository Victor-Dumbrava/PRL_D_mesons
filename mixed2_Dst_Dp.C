...
void mixed2_Dst_Dp() {
    ...
    TFile *output_file = new TFile(Form("mixed2_%s_%s.root", particle_type1, particle_type2), "RECREATE");
    if (!output_file || output_file->IsZombie()) {
        std::cerr << Form("Error: Unable to create output file 'mixed2_%s_%s.root'.\n", particle_type1, particle_type2);
        delete output_file;
        delete file1;
        delete file2;
    ...
    for (int i = 0; i < 6; ++i) {
        c2->cd(i + 1);
        TGraph *graph = ratio_plots[i]->GetCalculationOutputGraph();
        auto legend = new TLegend(0.22,0.76,0.42,0.91);
    ...
    c2->Update();
    c2->Write();
    c2->SaveAs("mixed2_Dst_Dp_2y5.pdf");
    output_file->Close();
    std::cout << "Done." << std::endl;
    delete output_file;
    delete file1;
