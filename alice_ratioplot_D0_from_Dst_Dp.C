...
void alice_ratioplot_D0_from_Dst_Dp() {
    ...
    TCut cuts1 = Form("%s_TRUEY >= 0.0 && %s_TRUEY < 1.0 && abs(Km_MC_MOTHER_ID) == 421 && abs(%s_MC_MOTHER_ID) == 413 && abs(Hp_MC_MOTHER_ID) == 421", particle_type1, particle_type1, particle_type1);
    ...
    TFile *output_file = new TFile(Form("alice_plotratio_%s_from_Dst_%s.root", particle_type1, particle_type2), "RECREATE");
    if (!output_file || output_file->IsZombie()) {
        std::cerr << Form("Error: Unable to create output file 'alice_plotratio_%s_from_Dst_%s.root'.\n", particle_type1, particle_type2);
        delete output_file;
        delete file1;
        delete file2;
        delete ratio_plot;
    ...
    c2->Update();
    c2->Write();
    c2->SaveAs("ratio_D0_from_Dst_Dp_0y1.pdf");
    output_file->Close();
