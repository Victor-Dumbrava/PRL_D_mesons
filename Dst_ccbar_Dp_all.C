...
void Dst_ccbar_Dp_all() {
    ...
    TFile *output_file = new TFile("Dst_ccbar_Dp_all_new.root", "RECREATE");
    if (!output_file || output_file->IsZombie()) {
        std::cerr << "Error: Unable to create output file 'Dst_ccbar_Dp_all_new.root'.\n";
        delete output_file;
        return;
    }
    ...
    if (!graphs.empty()) {
        ...
        new_graph->GetYaxis()->SetRangeUser(1.3, 2.85);
        new_graph->Draw("AP"); 
        legend->AddEntry(new_graph, labels[0], "elp");
        legend->SetBorderSize(0);
        legend->SetTextSize(0.05);
        new_graph->SetTitle("D^{*+} and D^{+} from c or #bar{c}");
    ...
    for (size_t i = 1; i < graphs.size(); ++i) {
        if (i == 3) {
            ...
            new_graph->GetYaxis()->SetRangeUser(1.3, 2.85);
            new_graph->Draw("AP"); 
            new_graph->SetTitle("D^{*+} from c or #bar{c},    D^{+} from feeddown of D^{*+}");
            new_graph->GetYaxis()->SetTitleOffset(0.8);
            new_graph->GetXaxis()->SetTitleOffset(0);
            new_graph->GetYaxis()->SetTitle("D^{*+} / D^{+}");
