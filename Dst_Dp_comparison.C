...
void Dst_Dp_comparison() {
    ...
    // boolean params are: false/true (for D0/Dst), false/true (for ccbar/other sources), false/true (for lchb/alice) respectively
    openInputFiles(graphs, true, false, false); // Dst, direct (ccbar), forward
    openInputFiles(graphs, true, false, true);
    openInputFiles(graphs, true, true, false); // Dst, indirect (not ccbar), forward
    openInputFiles(graphs, true, true, true);

    std::cout << "Starting processing." << std::endl;

    TFile *output_file = new TFile("comparison_Dst_Dp.root", "RECREATE");
    if (!output_file || output_file->IsZombie()) {
        std::cerr << "Error: Unable to create output file 'comparison_Dst_Dp.root'.\n";
        delete output_file;
        return;
    }
    ...
    c1->Write();
    c1->Draw();
    c1->SaveAs("comparison_Dst_Dp.pdf");
    output_file->Close();
    delete output_file;
