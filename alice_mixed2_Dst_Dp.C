...
void alice_mixed2_Dst_Dp() {
    ...
    TCut cuts1 = Form("%s_TRUEY >= 0.0 && %s_TRUEY < 1.0 && abs(Km_MC_MOTHER_ID) == 421 && abs(Hp_MC_MOTHER_ID) == 421 && abs(H_soft_MC_MOTHER_ID) == 413 && abs(%s_MC_MOTHER_ID) == 4", particle_type1, particle_type1, particle_type1);
    ...
    TFile *output_file = new TFile(Form("alice_mixed2_plotratio_%s_%s.root", particle_type1, particle_type2), "RECREATE");
    if (!output_file || output_file->IsZombie()) {
        std::cerr << Form("Error: Unable to create output file 'alice_mixed2_plotratio_%s_%s.root'.\n", particle_type1, particle_type2);
        delete output_file;
        delete file1;
        delete file2;
        delete ratio_plot;
    ...
    graph->GetYaxis()->SetTitle("D^{*+} / D^{+}");
    graph->SetTitle("D^{*+} from c or #bar{c}, D^{+} from D^{*0} or D^{*+}");
    ...
    c2->Update();
    c2->Write();
    c2->SaveAs("ratio_mixed2_Dst_Dp_0y1.pdf");
    output_file->Close();
