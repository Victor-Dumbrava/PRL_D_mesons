...
void ratioplot_D0_from_Dst_Dp() {
    ...
    for (int i = 0; i < num_rapidity_bins; ++i) {
        ...
        TCut cuts1 = Form("%s_TRUEY >= %f && %s_TRUEY < %f && abs(%s_MC_MOTHER_ID) == 413 && abs(Km_MC_MOTHER_ID) == 421 && abs(Hp_MC_MOTHER_ID) == 421", particle_type1, rapidity_bins[i], particle_type1, rapidity_bins[i+1], particle_type1);
    ...
    TFile *output_file = new TFile(Form("plotratio_%s_from_Dst_%s.root", particle_type1, particle_type2), "RECREATE");
    if (!output_file || output_file->IsZombie()) {
        std::cerr << Form("Error: Unable to create output file 'plotratio_%s_from_Dst_%s.root'.\n", particle_type1, particle_type2);
        delete output_file;
        delete file1;
        delete file2;
    ...
    for (int i = 0; i < 6; ++i) {
        ...
        graph->GetYaxis()->SetTitle("D^{0} / D^{+} (from D^{*+} #rightarrow D^{0} #pi^{+})");
    ...
    c2->Update();
    c2->Write();
    c2->SaveAs("ratios_D0_from_Dst_Dp_2y5.pdf");
    output_file->Close();
    std::cout << "Done." << std::endl;
    delete output_file;
    delete file1;
